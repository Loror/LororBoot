...
public class ApiClient {
    ...
    private void result(Responce responce, Class<?> classType, Observer observer) {
        if (responce.getCode() == 200) {
            try {
                Object bean = classType == String.class ? responce.toString() : jsonParser == null ? null : jsonParser.jsonToObject(responce.toString(), classType);
    ...
    private Object result(Responce responce, Class<?> classType) {
        if (responce.getCode() == 200) {
            try {
                return classType == String.class ? responce.toString() : jsonParser == null ? null : jsonParser.jsonToObject(responce.toString(), classType);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
